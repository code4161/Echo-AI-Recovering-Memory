...
export interface DialogueRule {
  id: string;
  /** Checked in array order, so put the specific rules first. */
  match: RegExp;
  emotion: Emotion;
  trust: number;
 ...
 *
 * A placeholder for a language model: keyword rules that pick a mood, a trust
...
/** Anti-spam: repeating yourself is gently noticed and earns nothing. */
